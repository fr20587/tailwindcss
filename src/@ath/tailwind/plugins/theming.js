...
const generateVariableColors = (theme) => {
    ...
    const customPropertiesWithOpacity = (name) => ({
        ...
        if (opacityValue) {
            return `rgba(var(--ath-${name}-rgb), ${opacityValue})`;
        }
        if (opacityVariable) {
            return `rgba(var(--ath-${name}-rgb), var(${opacityVariable}, 1))`;
        }
        return `rgb(var(--ath-${name}-rgb))`;
    };
...
const theming = plugin.withOptions((options) => ({
        ...
        const mapVariableColors = _.fromPairs(_.map(options.themes, (theme, themeName) => [
            ...
            _.fromPairs(_.flatten(_.map(flattenColorPalette(_.fromPairs(_.flatten(_.map(normalizeTheme(theme), (palette, paletteName) => [
                ...
                [`--ath-${e(key)}`, value],
                [`--ath-${e(key)}-rgb`, chroma(value).rgb().join(',')]
            ])))
        ]));
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) => {
            ...
            const background = theme(`ath.customProps.background.${colorScheme}`);
            const foreground = theme(`ath.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [
                        [`--ath-${e(key)}`, value],
                        [`--ath-${e(key)}-rgb`, chroma(value).rgb().join(',')]
                    ]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [
                        [`--ath-${e(key)}`, value],
                        [`--ath-${e(key)}-rgb`, chroma(value).rgb().join(',')]
                    ])))
                }
